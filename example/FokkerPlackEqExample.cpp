...
/**
 * @example FokkerPlackEqExample.cpp
 * 	
 ...
 * \f[\frac{d p(t,x)}{dt} = \Big(\frac{1}{2}\sigma^2 \frac{\partial^2 }{\partial x^2} - \mu  \frac{\partial^2 }{\partial x} + \gamma \Big) p(t,x) \f]
...
int main () {
  ...
  ForwardKolmogorowEquation fokker_plack_equation(process);

  auto fdmcn = fokker_plack_equation.solveAndSave(cnscheme, f, bcs, spatial_grid, temporal_grid, "fokker_plack_equation");
}
