...
namespace marian {
  ...
  DataFrame::DataFrame(std::string file_name,
		       char delimiter,
   ...
   */  
  double DataFrame::getNumber(const int col, const  int row) {
    return std::stod(data_[row-1][col-1]);
  }
   ...
   * \note May be used only when column names were defined.
   */ 
  double DataFrame::getNumber(const std::string col, const int row) {
    return std::stod(data_[row-1][column_names_[col]-1]);
  }
   ...
   * \note May be used only when column names and primary keys were defined.
   */ 
  double DataFrame::getNumber(const std::string col, const std::string row) {
    return std::stod(data_[primary_key_[row]-1][column_names_[col]-1]);
  }
